 ...
 * Change Logs:
 * Date           Author       Notes
 * 2023-11-16     Thread       the first version
 */
#define LOG_TAG "gy906"
...
int GY906_Init(void)
{
    ...
    // 初始化互斥锁
    temp_mutex = rt_mutex_create("temp_mutex", RT_IPC_FLAG_FIFO);
    if (temp_mutex == RT_NULL) {
        LOG_E("Failed to create temp_mutex");
        return RT_ERROR;
    }
    return RT_EOK;
}
