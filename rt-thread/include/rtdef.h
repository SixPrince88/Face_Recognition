...
#define INIT_EXPORT(fn, level)            \
    const char __rti_##fn##_name[] = #fn; \
    rt_used const struct rt_init_desc __rt_init_desc_##fn rt_section(".rti_fn." level) = { __rti_##fn##_name, fn };
#else
#define INIT_EXPORT(fn, level) \
 ...
 *
 * @def RT_ALIGN(size, align)
 * 返回在指定宽度上对齐的最连续大小。例如，RT_ALIGN(13, 4) 将返回 16。
 */
 ...
 */
#define RT_IPC_FLAG_FIFO 0x00 /**< FIFOed IPC. FIFO（先进先出）方式的IPC。@ref IPC. */
#define RT_IPC_FLAG_PRIO 0x01 /**< PRIOed IPC. 优先级（PRIO）方式的IPC。@ref IPC. */

#define RT_IPC_CMD_UNKNOWN 0x00 /**< unknown IPC command. 未知IPC命令 */
#define RT_IPC_CMD_RESET 0x01 /**< reset IPC object. 重置IPC对象 */
#define RT_IPC_CMD_GET_STATE 0x02 /**< get the state of IPC object. 获取IPC对象的状态 */
#define RT_IPC_CMD_SET_VLIMIT 0x03 /**< set max limit value of IPC value. 设置IPC值的最大限制值 */

#define RT_WAITING_FOREVER -1 /**< Block forever until get resource. 阻塞直到获取资源 */
#define RT_WAITING_NO 0 /**< Non-block. 非阻塞 */
