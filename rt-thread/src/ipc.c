...
/**
 * @brief  此函数将从事件对象接收一个事件。如果事件不可用，线程将等待指定的时间。
 *
 * @note   如果有线程在此信号量上挂起，此信号量对象列表中的第一个线程将被恢复，并执行线程调度（rt_schedule）。
 *         如果没有线程在此信号量上挂起，此信号量的计数值 sem->value 将增加1。
 *
 * @param    event 是要接收的事件对象的指针。
 *
 * @param    set 是您将为此事件标志设置的标志。
 *           您可以设置一个事件标志，或通过逻辑或操作设置多个标志。
 *
 * @param    option 是此接收事件的选项，它指示如何操作接收的事件。
 *           选项可以是以下值之一或多个，当选择多个值时，使用逻辑或操作。
 *           （注意：RT_EVENT_FLAG_OR 和 RT_EVENT_FLAG_AND 只能选择一个）：
 *
 *
 *               RT_EVENT_FLAG_OR           线程选择使用逻辑或接收事件。
 *
 *               RT_EVENT_FLAG_AND          线程选择使用逻辑或接收事件。
 *
 *               RT_EVENT_FLAG_CLEAR        当线程接收到相应的事件时，函数确定是否清除事件标志。
 *
 * @param    timeout 是超时时间（单位：一个操作系统节拍）。
 *
 * @param    recved 是接收的事件的指针。如果您不关心此值，可以使用 RT_NULL 设置。
 *
 * @return   返回操作状态。当返回值为 RT_EOK 时，操作成功。
 *           如果返回值是其他任何值，则表示信号量释放失败。
 */
    ...
    rt_uint8_t flag)
{
    ...
    for (temp = 0; temp < mq->max_msgs; temp++) {
        head = (struct rt_mq_message*)((rt_uint8_t*)mq->msg_pool + temp * (msg_align_size + sizeof(struct rt_mq_message)));
 ...
 *           消息队列标志可以是以下值之一：
 *
 *               RT_IPC_FLAG_PRIO          等待的线程将按照优先级排队。
 *
 *               RT_IPC_FLAG_FIFO          等待的线程将按照先入先出（FCFS）的方法排队
 *                                         （也称为先来先服务（FCFS）调度策略）。
 *
 *               注意：RT_IPC_FLAG_FIFO 是非实时调度模式。强烈建议使用 RT_IPC_FLAG_PRIO 来确保线程是实时的，
 *               除非您的应用程序关心先入先出的原则，并且您明确知道参与此消息队列的所有线程都将变为非实时线程。
 *
 * @return   返回指向消息队列对象的指针。当返回值为 RT_NULL 时，表示创建失败。
 *
 * @warning  此函数不能在中断上下文中调用。您可以使用宏 RT_DEBUG_NOT_IN_INTERRUPT 进行检查。
 */
...
/**
 * @brief    该函数将设置消息队列对象的额外属性。
 *
 * @note     当前此函数仅支持 RT_IPC_CMD_RESET 命令，用于重置消息队列。
 *
 * @param    mq 是指向消息队列对象的指针。
 *
 * @param    cmd 是用于配置消息队列属性的命令。
 *
 * @param    arg 是执行命令的函数参数。
 *
 * @return   返回操作状态。当返回值为 RT_EOK 时，操作成功。
 *           如果返回值是其他值，则表示此函数执行失败。
 */
