...
/**
 * @brief  此函数将初始化一个内存池对象，通常用于静态对象。
 *
 * @param  mp 是内存池对象。
 *
 * @param  name 是内存池的名称。
 *
 * @param  start 是内存池的起始地址。
 *
 * @param  size 是内存池的总大小。
 *
 * @param  block_size 是每个块的大小。
 *
 * @return RT_EOK
 */
...
/**
 * @brief 此函数将从内存池中分配一个内存块。
 *
 * @param mp 是内存池对象。
 *
 * @param time 是分配内存的最大等待时间。
 *             - 0 表示不等待，立即分配内存。
 *
 * @return 分配的内存块，如果分配失败则返回 RT_NULL。
 */
