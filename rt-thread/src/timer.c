 ...
 *
 * @param time 是定时器的超时节拍数
 *
 *        注意：最大超时节拍数应不超过 (RT_TICK_MAX/2 - 1)。
 *
 * @param flag 是定时器的标志。如果设置了标志中的一个或多个值，定时器将根据标志的选定值调用超时函数。
 *
 *          RT_TIMER_FLAG_ONE_SHOT          一次性定时
 ...
 *
 *        注意：
 *        您可以使用 "|" 逻辑运算符使用多个值。默认情况下，系统将使用 RT_TIME_FLAG_HARD_TIMER。
 *
 * @return 创建的定时器对象
 */
